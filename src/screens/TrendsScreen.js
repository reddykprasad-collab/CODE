...
const TRIGGER_TIPS = {
  'Poor sleep': "Try keeping your wake time consistent even on weekends — irregular sleep schedules are a stronger trigger than total sleep hours.",
  'Stress': "Even 5 minutes of slow breathing before stressful moments can reduce the cortisol spike that precedes attacks for many people.",
  'Dehydration': "Front-load your water intake. Most people are already dehydrated by the time they feel thirsty.",
  'Caffeine change': "The trigger is usually the change, not the caffeine itself. Keeping intake consistent beats going cold turkey.",
  'Skipped meal': "Blood sugar dips are a reliable trigger. Keeping something small with you — a handful of nuts — is more effective than eating more at meals.",
  'Weather change': "Weather triggers are hard to avoid, but the risk forecast in this app can help you prepare. Have your rescue medication accessible on high-risk days.",
  'Hormonal': "Tracking your cycle alongside migraines often reveals a 2–3 day window each month. Some doctors recommend a targeted prevention strategy for that window.",
  'Bright light': "Blue-light blocking glasses worn in the evening — not just during attacks — reduce accumulated light sensitivity for many patients.",
  'Alcohol': "Red wine and dark spirits are higher-risk than clear spirits or beer. The trigger is often congeners, not just alcohol content.",
  'Physical exertion': "Warming up gradually and staying hydrated before intense exercise significantly reduces exertion-triggered attacks.",
};
...
export default function TrendsScreen({ navigation }) {
  ...
  async function handleShareImpact() {
    const migraineDays = new Set(thisMonth.filter(e => e.hadMigraine).map(e => new Date(e.date).toDateString())).size;
    const daysTracked = new Set(thisMonth.map(e => new Date(e.date).toDateString())).size;
    const impacts = Object.entries(
      thisMonth.filter(e => e.hadMigraine).reduce((acc, e) => {
        (e.functionalImpact || []).forEach(i => { acc[i] = (acc[i] || 0) + 1; });
        return acc;
      }, {})
    ).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([k]) => k);

    const lines = [
      "Here's what migraines have looked like for me this month:",
      '',
      `• ${migraineDays} migraine day${migraineDays !== 1 ? 's' : ''} out of ${daysTracked} days tracked`,
    ];

    if (avgSeverity) lines.push(`• Average pain level: ${avgSeverity}/10`);
    if (impacts.length > 0) lines.push(`• Most common impacts: ${impacts.join(', ').toLowerCase()}`);
    if (mohStatus.primaryDays > 0) lines.push(`• Used rescue medication ${mohStatus.primaryDays} day${mohStatus.primaryDays !== 1 ? 's' : ''} this month`);
    if (midasScores.length > 0 && midasScores[0].total >= 11) {
      lines.push('', `My disability score (MIDAS): ${midasScores[0].total} — ${midasScores[0].label} disability level.`);
    }

    lines.push(
      '',
      "Migraines aren't just headaches. They're a neurological condition that affects my ability to work, socialize, and function. I'm tracking my patterns and working with my doctor to manage them.",
      '',
      'Generated by Migraine Companion'
    );
  ...
  const mohStatus = useMemo(() => computeMOHStatus(thisMonth), [thisMonth]);
  const mohDays = mohStatus.primaryDays; // keep for any existing references
        ...
        <View style={styles.headerRow}>
          ...
          {entries.length > 0 && (
            <View style={{ flexDirection: 'row', gap: 14, alignItems: 'center' }}>
              <TouchableOpacity onPress={handleShareImpact} style={styles.exportBtn} accessibilityRole="button" accessibilityLabel="Share my migraine impact">
                <Feather name="share-2" size={16} color={colors.slateMid} />
          ...
          <>
            ...
            {topTriggers.length > 0 && (
              <>
                ...
                {topTriggers[0][1] >= 3 && TRIGGER_TIPS[topTriggers[0][0]] && (
                  <View style={styles.triggerTipCard}>
                    <View style={styles.triggerTipHeader}>
                      <Feather name="zap" size={13} color={colors.lav} />
                      <Text style={styles.triggerTipEyebrow}>Based on your top trigger</Text>
                    </View>
                    <Text style={styles.triggerTipText}>{TRIGGER_TIPS[topTriggers[0][0]]}</Text>
                  </View>
                )}
              </>
            )}
            ...
            {/* ── MOH warning ── */}
            {mohStatus.mohRisk !== 'none' && (
              <View style={[styles.mohCard, mohStatus.mohRisk === 'approaching' && { backgroundColor: '#1C1508', borderColor: '#3D2E08' }]}>
                <View style={styles.mohIconWrap}>
                  <Feather
                    name={mohStatus.mohRisk === 'exceeded' ? 'alert-triangle' : 'alert-circle'}
                    size={18}
                    color={mohStatus.mohRisk === 'exceeded' ? colors.terra : colors.amber}
                  />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.mohTitle}>
                    {mohStatus.mohRisk === 'exceeded'
                      ? `Frequent rescue medication use`
                      : `Approaching rescue medication limit`}
                  </Text>
                  <Text style={styles.mohBody}>
                    {mohStatus.mohRisk === 'exceeded'
                      ? `You used ${mohStatus.primaryClass === 'triptan' ? 'triptans' : 'pain relievers'} on ${mohStatus.primaryDays} days this month. Using them more than ${mohStatus.primaryThreshold} days a month can gradually increase headache frequency. Worth raising with your doctor.`
                      : `You've used rescue medication ${mohStatus.primaryDays} of ${mohStatus.primaryThreshold} days this month. Most people find headaches become harder to control beyond that limit.`}
                  </Text>
                </View>
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  triggerTipHeader: { flexDirection: 'row', alignItems: 'center', gap: 5, marginBottom: 6 },
  triggerTipEyebrow: { fontFamily: fonts.bodySemiBold, fontSize: textSize.fine, color: colors.lav, letterSpacing: 0.3 },
  triggerTipText: { fontFamily: fonts.body, fontSize: textSize.body, color: colors.slateMid, lineHeight: 22 },
